...
public class RecentWarLogHandler extends AbstractHandler {
    @Override
    public HttpResponse realhandle(HttpExchange exchange) throws Exception {
        ...
        try (PreparedStatement stmt = conn.prepareStatement("select w.server, w.guild, w.start_time, w.end_time, t.defender, t.terr_name, t.acquired, w.id from war_log w left join terr_log t on w.terr_entry=t.id order by w.start_time desc limit 5 offset ?;")) {
            if (_offset != null) {
                stmt.setInt(1, Integer.parseInt(_offset));
            } else {
                stmt.setInt(1, 0);
            }
