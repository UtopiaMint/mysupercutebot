...
public class PlayerWarStatsCommand implements CommandHandler {
    ...
    public PaginatedMessage process(MessageReceivedEvent evt, String cmd, String rest, PaginatedMessage pmsg) {
        ...
        if (data.has("wars") && data.getJSONArray("wars").length() > 0) {
            ...
            int survived = data.getJSONObject("aggregated").getInt("survived");
            ...
            sb.append("\nSurvived Wars: ").append(survived).append(" / ").append(total);
            sb.append(" (").append(String.format("%.2f", 100.0 * survived / total)).append("%)");
            sb.append("\n\n");
            ...
            action = evt.getChannel().sendMessage("There are no war logs recorded for this player.");
            action.complete();
            return null;
        }
    }
